...
const Workshop = () => {
  ...
  const settings = {
    ...
    slidesToScroll: 1,
    swipeToSlide: true,
    responsive:[{breakpoint: 600, settings: {slidesToShow:1}},{breakpoint: 1024, settings: {slidesToShow:2}},]
  };
      ...
      <Slider {...settings}>
        {images.map((image, index) => (
          <div key={index}>
            ...
            <img className="rounded-tr-[4em] h-[50vw] sm:h-[30vw] md:h-[30vw] md:max-h-[40vh] md:max-w-[50vw] px-1"src={image.src} alt={`Slide ${index + 1}`} />
            <div className='text-lg mt-4 font-poppins font-bold pl-2'>{image.name}</div>
            <div className='flex flex-row mt-4 font-semibold pl-2'>
              <div className='border-2 text-sm font-poppins w-12 md:h-6 pl-1 border-orange-500'>₹{image.prize}</div>
